...
class BottleCounter {
  private lazy var userDefaults = BottleCounterUserDefaults()
  private lazy var apiClient = APIClient(apiService: APIService(baseURL: "https://oceanhero.today/api/"), apiParser: APIParser())
  ...
  private var finalBottleCounter: Int {
    get {
      let value = userDefaults.currentBottleCounter
    ...
    set {
      userDefaults.currentBottleCounter = newValue
    }
  }
...
extension BottleCounter {
  func start() {
    ...
    if userDefaults.animationFromCurrentBottleCounterValue {
      startBottleCounter = finalBottleCounter
      currentBottleCounter = finalBottleCounter
    } else {
      userDefaults.animationFromCurrentBottleCounterValue = true
