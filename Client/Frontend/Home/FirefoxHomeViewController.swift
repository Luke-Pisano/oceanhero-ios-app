  ...
  private var viewHasAppeared = false
    ...
    override func viewDidLoad() {
        ...
        Section.allCases.forEach {
          guard !$0.cellIdentifier.isEmpty else {
            return
          }
        ...
        self.collectionView?.register(ASHeaderBottleView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "HeaderBottle")
        collectionView?.keyboardDismissMode = .onDrag
        collectionView?.backgroundColor = .clear

      //TODO: turn on when you want to display pop up about of default Browser Card
...
//        }
  ...
  override func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
    delegate?.firefoxHomeWillBeginDragging()
  }
}
  ...
  func refreshBottle() {
    ...
    private func updateTotalBottle(value: String) {
      collectionView.visibleSupplementaryViews(ofKind: UICollectionView.elementKindSectionHeader).forEach { view in
        guard let view = view as? ASHeaderBottleView else {
          return
        }

        updateTotalBottle(for: view, value: value)
      }
    }

    func updateTotalBottle(for view: ASHeaderBottleView) {
        updateTotalBottle(for: view, value: bottleCounter.currentValue)
    }

    private func updateTotalBottle(for view: ASHeaderBottleView, value: String) {
    ...
    enum Section: Int, CaseIterable {
      case topBottle
        ...
        var footerHeight: CGSize {
            switch self {
            case .topBottle, .pocket, .jumpBackIn, .recentlySaved: return .zero
        ...
        var cellType: UICollectionViewCell.Type {
            switch self {
            case .topBottle: return UICollectionViewCell.self
...
extension FirefoxHomeViewController: UICollectionViewDelegateFlowLayout {
  private func configSectionHeader(_ view: ASHeaderView, for section: Section) {
    switch section {
    case .topBottle:
      break
    case .pocket:
    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        case UICollectionView.elementKindSectionHeader:
            ...
            case .topBottle:
              let view = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "HeaderBottle", for: indexPath) as! ASHeaderBottleView
              return view
            case .pocket, .jumpBackIn, .recentlySaved, .topSites, .libraryShortcuts:
              let view = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "Header", for: indexPath) as! ASHeaderView
                ...
                configSectionHeader(view, for: Section(indexPath.section))
                return view
        }
        default:
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
      ...
      case .topBottle:
        return CGSize(width: .zero, height: 176)
      case .pocket:
...
extension FirefoxHomeViewController: DataObserverDelegate {
    ...
    @objc fileprivate func longPress(_ longPressGestureRecognizer: UILongPressGestureRecognizer) {
        ...
        case .topBottle, .libraryShortcuts, .jumpBackIn, .recentlySaved:
            return
        }
    }
    ...
    func selectItemAtIndex(_ index: Int, inSection section: Section) {
        ...
        case .topBottle, .topSites, .libraryShortcuts, .jumpBackIn, .recentlySaved:
            return
        }
        if let site = site {
...
extension FirefoxHomeViewController: HomePanelContextMenu {
    ...
    func getContextMenuActions(for site: Site, with indexPath: IndexPath) -> [PhotonActionSheetItem]? {
        ...
        switch Section(indexPath.section) {
        case .topBottle, .pocket, .libraryShortcuts, .jumpBackIn, .recentlySaved: break
